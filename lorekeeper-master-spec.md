...
- **Plot Archetype tool:** Several bugs reported — audit the full flow; check that the Claude API call prompt is correct, that the result is displayed properly, and that edge cases (no scenarios, missing data) are handled.
- **Relationship Map tool:** Several bugs reported — audit node rendering, edge drawing, drag behavior, and save/load of relationship data. Known fragile areas: canvas coordinate transforms, node overlap, edge label display.
