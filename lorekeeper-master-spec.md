...
**Last updated: September 2, 2026 (session 35d)**
...
### Verification status
**Built and compile-verified; not yet verified against real export output.** The builders are strictly stricter than the code they replaced, so the failure mode would be a field missing from an export rather than a bad field reaching the site. Outstanding check, for whoever picks this up:

- Export a character to disk: no `relPath` anywhere under `portraits`; `companion_profile_banner_image` is a `{id}` object or absent, never `undefined`
- Export a collection: `description` holds the text, `definition` is absent
- Export a lorebook: `selected_chapter_index` and `collaboration_type` both present
- Run one world Export ZIP and spot-check one file of each type
